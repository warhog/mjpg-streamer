...
void send_request_and_process_response(struct extractor_state * state) {
    ...
    char buf[18 + strlen(state->url)];
    memset(buf, '\0', sizeof(buf));
    snprintf(buf, sizeof(buf), "GET %s HTTP/1.0\r\n\r\n", state->url);
    
    // send request
    send(state->sockfd, buf, sizeof(buf), 0);
...
fprintf(stderr, " ---------------------------------------------------------------\n" \
                ...
                " [-u | --url].............: url, defaults to /mjpg/video.mjpg\n"
    ...
    while (TRUE) {
        ...
        if (c=='?'){
            ...
            case 'p' :
                ...
                break;
            case 'b' :
                state->boundary.string = strdup(optarg);
                break;
            case 'u' :
                free(state->url);
                state->url = strdup(optarg);
                break;
            }
    }
